...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the product demand base table")
...
def main() -> None:
    ...
    output_path = args.output or config["paths"]["product_demand_base"]
    ...
    spark = get_spark("product-demand-base", jdbc_jar_path=args.jdbc_jar)
    ...
    featured_df.show(args.limit, truncate=False)
    featured_df.write.mode("overwrite").parquet(output_path)

    print(f"Saved product demand base to {output_path}")
    spark.stop()
