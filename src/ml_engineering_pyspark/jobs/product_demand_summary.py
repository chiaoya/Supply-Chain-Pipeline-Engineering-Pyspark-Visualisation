...
from ml_engineering_pyspark.transforms.demand_processing import (
    ...
    aggregate_product_demand_summary,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the product demand summary table")
...
def main() -> None:
    ...
    output_path = args.output or config["paths"]["product_demand_summary"]
    ...
    spark = get_spark("product-demand-summary", jdbc_jar_path=args.jdbc_jar)
    ...
    summary_df = aggregate_product_demand_summary(featured_df)

    summary_df.show(args.limit, truncate=False)
    summary_df.write.mode("overwrite").parquet(output_path)

    print(f"Saved product demand summary to {output_path}")
    spark.stop()
